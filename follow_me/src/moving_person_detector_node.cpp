...
class moving_persons_detector
{
  ...
  public:
	moving_persons_detector()
	{
		...
		pub_moving_persons_detector_marker = n.advertise<visualization_msgs::Marker>(
		    "moving_person_detector", 1); // Preparing a topic to publish our results. This will be used by the visualization tool rviz
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
	...
	void detect_motion()
	{
		...
		for (int loop = 0; loop < nb_beams; loop++) // loop over all the hits
			if (abs(background[loop] - range[loop]) < dynamic_threshold)
	...
	void perform_clustering()
	{
		...
		for (int loop = 1; loop < nb_beams; loop++) // loop over all the hits
			if (distancePoints(current_scan[loop - 1], current_scan[loop]) < cluster_threshold) {
				...
				// textual display
				ROS_INFO("cluster[%i]: [%i](%f, %f) -> [%i](%f, %f), size: %f, dynamic: %i", nb_cluster, cluster_start[nb_cluster],
				         current_scan[cluster_start[nb_cluster]].x, current_scan[cluster_start[nb_cluster]].y, cluster_end[nb_cluster],
				         current_scan[cluster_end[nb_cluster]].x, current_scan[cluster_end[nb_cluster]].y, cluster_size[nb_cluster],
				         cluster_dynamic[nb_cluster]);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
	void detect_moving_legs()
	{
		...
		{
			float distance = distancePoints(current_scan[cluster_end[loop]], current_scan[cluster_start[loop]]);
			if (cluster_dynamic[loop] / cluster_size[loop] > dynamic_threshold / 100 && distance < leg_size_max && distance > leg_size_min) {
				// we update the moving_leg_detected table to store the middle of the moving leg
				moving_leg_detected[nb_moving_legs_detected].x = (current_scan[cluster_end[loop]].x + current_scan[cluster_start[loop]].x) / 2;
				moving_leg_detected[nb_moving_legs_detected].y = (current_scan[cluster_end[loop]].y + current_scan[cluster_start[loop]].y) / 2;
				moving_leg_detected[nb_moving_legs_detected].z = (current_scan[cluster_end[loop]].z + current_scan[cluster_start[loop]].z) / 2;
		...
		for (int loop_leg1 = 0; loop_leg1 < nb_moving_legs_detected; loop_leg1++) {                 // loop over all the legs
			for (int loop_leg2 = loop_leg1 + 1; loop_leg2 < nb_moving_legs_detected; loop_leg2++) { // loop over all the legs
				if (distancePoints(moving_leg_detected[loop_leg1], moving_leg_detected[loop_leg2]) < legs_distance_max) {
					...
					moving_persons_detected[nb_moving_persons_detected].z = (moving_leg_detected[loop_leg1].z + moving_leg_detected[loop_leg2].z) / 2;

					// textual display
					ROS_INFO("moving person detected[%i]: leg[%i]+leg[%i] -> (%f, %f)", nb_moving_persons_detected, loop_leg1, loop_leg2,
					         moving_persons_detected[nb_moving_persons_detected].x, moving_persons_detected[nb_moving_persons_detected].y);
